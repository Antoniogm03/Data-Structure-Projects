...
tree<T>::preorder_iterator::operator++() {
    if (!current.left_child().is_null()) {
    ...
    } else if (!current.right_sibling().is_null()) {
        current = current.right_sibling();
    } else {
        while (!current.parent().is_null() && current.parent().right_sibling().is_null()) {
            level--;
        ...
        if (current.parent().is_null()) {
            ...
            current = current.parent().right_sibling();
            level--;
        }
    }
    return *this;
...
tree<T>::begin_preorder() {
    tree<T>::preorder_iterator it(get_root());
    return it;
}
...
typename tree<T>::preorder_iterator tree<T>::end_preorder() {
    node nulo;
    tree<T>::preorder_iterator it(nulo);
    return it;
}
...
tree<T>::cbegin_preorder() const {
    const tree<T>::const_preorder_iterator it(get_root());
    return it;
}
...
tree<T>::cend_preorder() const {
    node nulo;
    tree<T>::const_preorder_iterator it(nulo);
    return it;
}